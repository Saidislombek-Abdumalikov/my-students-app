...
export const StudentTimeline: React.FC<StudentTimelineProps> = ({ studentId }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {events.length === 0 ? (
        <p className="text-xs text-slate-500">No activity recorded yet for this student.</p>
      ) : (
        <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
          {events.map((ev) => (
            <div key={ev.id} className="relative flex items-start space-x-3 group">
              {/* Dot Icon */}
              <div className="absolute -left-6 top-0.5 p-1 rounded-full bg-white border border-slate-200 shadow-md">
                {ev.icon}
              </div>

              {/* Event Content Box */}
              <div className="flex-1 glass-panel p-3.5 rounded-xl border border-slate-200 space-y-1 hover:border-slate-300 transition-colors">
                ...
                <p className="text-[10px] text-slate-500 font-mono pt-1">{ev.date}</p>
              </div>
            </div>
          ))}
        </div>
