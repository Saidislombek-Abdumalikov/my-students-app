    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="flex items-center space-x-4">
          <div className="p-3 rounded-xl bg-brand-950/80 border border-brand-800/50 text-brand-400">
            <TrendingUp className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-medium text-slate-400">Group Average Presence</p>
            <h3 className="text-xl font-bold text-slate-100">{overallAvgRate}%</h3>
            <p className="text-[10px] text-emerald-400 font-medium">Target: &gt; 85%</p>
          </div>
        </Card>

        <Card className="flex items-center space-x-4">
          <div className="p-3 rounded-xl bg-sky-950/80 border border-sky-800/50 text-sky-400">
            <CalendarCheck className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-medium text-slate-400">Total Lessons Held</p>
            <h3 className="text-xl font-bold text-slate-100">{lessons.length}</h3>
        ...
        <Card className="flex items-center space-x-4">
          ...
          <div>
            <p className="text-xs font-medium text-slate-400">Students At Risk (&lt;75%)</p>
            <h3 className="text-xl font-bold text-slate-100">{atRiskStudents.length}</h3>
            <p className="text-[10px] text-rose-400 font-medium">Needs Teacher Follow-up</p>
          </div>
        </Card>
      </div>
      ...
      {atRiskStudents.length > 0 && (
        <Card className="bg-rose-950/20 border-rose-800/40 p-4 space-y-2">
      ...
      <Card className="space-y-3">
        <h3 className="text-sm font-bold text-slate-100">Student Attendance Distribution</h3>
          ...
          {studentStats.map((item) => (
            ...
            >
              <div className="space-y-0.5">
                <h4 className="text-xs font-bold text-slate-200">{item.student.fullName}</h4>
                <p className="text-[11px] text-slate-400">
                  Present: <span className="text-emerald-400 font-semibold">{item.present}</span> • Late: <span className="text-amber-400 font-semibold">{item.late}</span> • Absent: <span className="text-rose-400 font-semibold">{item.absent}</span> • Excused: <span className="text-sky-400 font-semibold">{item.excused}</span>
                </p>
              </div>
              ...
              <div className="flex items-center space-x-3 w-full sm:w-48">
                ...
                <span className="text-xs font-bold text-slate-200 w-10 text-right">{item.rate}%</span>
              </div>
            </div>
          ))}
        </div>
