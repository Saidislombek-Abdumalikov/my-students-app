...
import { Users, GraduationCap, CalendarCheck, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const DashboardPage: React.FC = () => {
  ...
  // Unpaid Debtor Students (To'lashi kerak bo'lganlar)
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Darslar va o'quvchilar to'lov holati nazorati.
          </p>
        </div>
          ...
          <div className="space-y-3">
            {groups.map((g) => (
              <div key={g.id} className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
                ...
                <p className="text-[11px] text-slate-400 font-mono">{g.scheduleDescription}</p>
              </div>
            ))}
          </div>
        </Card>
