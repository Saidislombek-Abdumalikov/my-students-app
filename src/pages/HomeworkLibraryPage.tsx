      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 glass-panel p-3 rounded-xl">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 focus:outline-none focus:border-brand-500"
          />
        </div>
              ...
              <div className="space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h3 className="text-base font-bold text-slate-900 mt-1 group-hover:text-purple-600 transition-colors">
                      {item.title}
                    </h3>
                  </div>
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1.5 text-xs">
                  <span className="font-semibold text-slate-600">Included Tasks:</span>
                  {item.tasks.map((t, idx) => (
                    <p key={idx} className="text-slate-500 text-[11px] truncate">
                      • <strong className="text-emerald-600">[{t.taskType}]</strong> {t.title}
                    </p>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {item.tags.map((tag, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-0.5 text-[10px] font-mono rounded bg-purple-50 text-purple-700 border border-purple-200 flex items-center gap-1"
                    >
      ...
      {itemToPreview && (
        ...
        >
          <div className="space-y-3 text-xs">
            <h4 className="font-bold text-slate-800">Tasks Breakdown ({itemToPreview.tasks.length}):</h4>
            {itemToPreview.tasks.map((t, idx) => (
              <div key={idx} className="p-3 bg-slate-50 rounded-lg border border-slate-200 space-y-1">
                <p className="font-bold text-slate-900">
                  Task #{idx + 1}: {t.title} <span className="text-emerald-600">[{t.taskType}]</span>
                </p>
